...
const std::vector<Vertex> vertices = {{{-0.5f, -0.5f, 0.f}, {1.f, 0.f, 0.f}, {1.f, 0.f}},
...
class GukVulkanEngine
{
  ...
  private:
    ...
    {
        auto app = reinterpret_cast<GukVulkanEngine*>(glfwGetWindowUserPointer(window));
    ...
    void createInstance()
    {
        ...
        appInfo.pApplicationName = "Guk Vulkan Engine";
    ...
    void createSwapChain()
    {
        ...
        VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabiliteis);

        uint32_t imageCount = swapChainSupport.capabiliteis.minImageCount + 1;

        if (swapChainSupport.capabiliteis.maxImageCount > 0 &&
            imageCount > swapChainSupport.capabiliteis.maxImageCount) {
            imageCount = swapChainSupport.capabiliteis.maxImageCount;
        }
        ...
        createInfo.preTransform = swapChainSupport.capabiliteis.currentTransform;
    ...
    void createTextureImage()
    {
        ...
        stbi_uc* pixels = stbi_load("textures/blender_uv_grid_2k.png", &texWidth, &texHeight,
    ...
    void transitionImageLayout(VkImage image, VkFormat foramt, VkImageLayout oldLayout,
                      ...
                      VkBuffer& buffer, VkDeviceMemory& bufferMemory)
    {
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate buffer memeory!");
        }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
...

        {
            ...
            depthStencilBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
    ...
    void updateUniformBuffer(uint32_t currentImage)
    {
        ...
        ubo.model =
            glm::rotate(glm::mat4(1.0f), time * glm::radians(90.f), glm::vec3(0.f, 0.f, 1.f));
        ubo.view = glm::lookAt(glm::vec3(2.f, 2.f, 2.f), glm::vec3(0.f, 0.f, 0.f),
                               glm::vec3(0.f, 0.f, 1.f));
        ubo.proj = glm::perspective(
            glm::radians(45.f), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 10.0f);
        ubo.proj[1][1] *= -1;
    ...
    void drawFrame()
    {
        ...
        if (vkQueueSubmit2(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) !=
            VK_SUCCESS) {
    ...
    VkSurfaceFormatKHR
    chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableForamts)
    {
        for (const auto& availableForamt : availableForamts) {
            if (availableForamt.format == VK_FORMAT_B8G8R8A8_SRGB &&
                availableForamt.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
    ...
    SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device)
    {
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, surface, &details.capabiliteis);
    ...
    bool isDeviceSuitsble(VkPhysicalDevice device)
    {
